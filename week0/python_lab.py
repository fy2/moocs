...
three_digits_set = {d1*(base**0) + d2*(base**1) + d3*(base**2) for d1 in digits for d2 in digits for d3 in digits}
...
LofL_sum = sum([sum(s) for s in LofL]) # use form: sum([sum(...) ... ])
...
listdict2dict = { names[n]:d[n] for n in range(0,len(names)) if n in d }
